  ...
  return (
    <article className="group rounded-lg border border-border bg-white p-5 hover:shadow-sm hover:border-[#d1d5db] transition">
      <div className="flex items-center gap-2 text-xs">
        <span className="rounded-full bg-[#eef2f7] px-2.5 py-1 text-xs font-semibold text-[#005274]">{article.section ?? "Research Article"}</span>
      ...
      <Link href={`/articles/${article.id}`} className="mt-3 block">
        <h3 className="font-display text-base font-bold leading-6 text-[#005274] group-hover:text-[#0066cc] group-hover:underline underline-offset-4 line-clamp-3">{article.title}</h3>
      </Link>
      ...
      <div className="mt-3 flex items-center gap-3">
        <Link href={`/articles/${article.id}`} className="text-sm font-semibold text-[#0066cc] hover:text-[#0052a3]">
          Read article
        </Link>
        {article.doi && (
